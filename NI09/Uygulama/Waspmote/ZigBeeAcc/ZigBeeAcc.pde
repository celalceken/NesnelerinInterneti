...
void loop()
{
  ...
  snprintf( frameMessageBuffer, sizeof(frameMessageBuffer),  ":%d:%d:%d:%d:%d\n",temp, (int)PWR.getBatteryLevel(),ACC.getX(), ACC.getY(), ACC.getZ());  
  ...
  error = xbeeZB.send( RX_ADDRESS, frameMessageBuffer ); 
   //error = xbeeZB.send(  frameMessageBuffer); 
